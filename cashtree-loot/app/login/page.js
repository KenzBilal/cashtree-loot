...
export default function LoginPage() {
  ...
  return (
    <div style={{
      ...
      <style>{`
        @keyframes fillUp { 0% { width: 0%; opacity: 0; } 8% { opacity: 1; } 100% { width: 100%; opacity: 1; } }
      ...
      <div style={{ position: 'fixed', top: '10%', left: '50%', transform: 'translateX(-50%)', width: 'min(500px, 100vw)', height: '500px', background: 'radial-gradient(circle, rgba(0,255,136,0.07) 0%, transparent 65%)', pointerEvents: 'none', zIndex: 0 }} />

      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }} onClick={() => setShowModal(false)}>
          <div style={{ background: '#0a0a0a', border: '1px solid #222', borderRadius: '22px', padding: '36px 28px', maxWidth: '320px', width: '100%', textAlign: 'center', boxShadow: '0 0 60px rgba(239,68,68,0.12)', animation: 'modalIn 0.28s cubic-bezier(0.16,1,0.3,1)' }} onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', color: '#444', fontSize: '12px', marginTop: '16px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ width: '100%', maxWidth: '390px', position: 'relative', zIndex: 1, animation: 'fadeUp 0.5s cubic-bezier(0.16,1,0.3,1)' }}>
        <div style={{ background: '#08080c', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '24px', padding: '38px 30px', boxShadow: '0 24px 60px -12px rgba(0,0,0,0.8)' }}>
          <div style={{ textAlign: 'center', marginBottom: '36px' }}>
            ...
            <p style={{ color: '#444', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1.5px', marginTop: '14px' }}>Secure Partner Portal</p>
          </div>
