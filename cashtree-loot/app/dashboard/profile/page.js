...
export default async function ProfilePage() {
  ...
  const containerStyle = { paddingBottom: '100px', animation: 'fadeIn 0.5s ease-out' };
  const pageHeader = { marginBottom: '30px', textAlign: 'center' };
  const idCard = {
    ...
    boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.7)', marginBottom: '30px'
  };
  const statBadge = {
    background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)',
    borderRadius: '16px', padding: '16px', flex: 1, textAlign: 'center', backdropFilter: 'blur(10px)'
  };
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={pageHeader}>
        <div style={{ fontSize: '10px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '3px', color: '#00ff88', marginBottom: '8px' }}>
          Personal Terminal
        </div>
        <h1 style={{ fontSize: '32px', fontWeight: '900', color: '#fff', margin: 0, textShadow: '0 0 30px rgba(255,255,255,0.1)' }}>
          My Profile
        </h1>
      </div>
      ...
      {/* ID CARD */}
      <div style={idCard}>
        <div style={{ position: 'absolute', top: '-80px', right: '-80px', width: '200px', height: '200px', background: 'radial-gradient(circle, #00ff88 0%, transparent 70%)', opacity: 0.15, filter: 'blur(40px)' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{
              ...
              border: '4px solid #111', boxShadow: '0 0 0 4px rgba(0,255,136,0.3)'
            }}>
            ...
            <div style={{
              ...
              borderRadius: '50%', border: '3px solid #111', boxShadow: '0 0 10px #00ff88'
            }} />
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            {/* Show email warning badge if no email */}
            {!account.email && (
              <div style={{
          ...
          <div style={{ display: 'flex', gap: '12px', width: '100%', marginTop: '10px' }}>
            <div style={statBadge}>
              ...
              <div style={{ fontSize: '18px', fontWeight: '900', color: '#fff' }}>₹{totalEarned.toLocaleString()}</div>
            </div>
            <div style={statBadge}>
              <div style={{ fontSize: '10px', color: '#888', textTransform: 'uppercase', fontWeight: '700', marginBottom: '4px' }}>Member Since</div>
              <div style={{ fontSize: '18px', fontWeight: '900', color: '#fff' }}>{new Date(account.created_at).getFullYear()}</div>
            </div>
          </div>
